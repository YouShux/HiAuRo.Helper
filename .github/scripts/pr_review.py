...
def get_pr_body() -> str:
    ...
    try:
        ...
        print(f"获取 PR 描述失败（非致命）: {e}")
        return ''


def call_deepseek(diff: str, pr_body: str, api_key: str, model: str, api_base: str) -> dict:
...
审查时，优先参考 PR 作者提供的修改说明。如果作者对修改给出了合理理由，不应判定为问题。
只有在修改会明显导致 Bug、性能退化或安全风险时，才标记为 critical。
...
包含一个 "files_changed" 字段，列出变更涉及的文件名（去重）"""
    ...
    user_prompt = ""
    if pr_body:
        user_prompt += f"PR 作者修改说明：\n{pr_body}\n\n"
    user_prompt += f"以下为代码变更 (diff)：\n\n{diff}"
    if truncated:
...
def main():
    ...
    pr_body = get_pr_body()
    if pr_body:
        print(f"PR 描述: {len(pr_body)} 字符")

    # 调用 DeepSeek
    try:
        response = call_deepseek(diff, pr_body, api_key, model, api_base)
    except Exception:
