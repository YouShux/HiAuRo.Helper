...
完全独立，**零 HiAuRo 依赖**。可被任何 Dalamud 插件引用。
...
class MyContext : IHelperContext
{
    ...
    public bool HasStatusOnTarget(uint id) { /* 目标 DoT 检查 */ }
}
...
| API | 说明 |
|-----|------|
| `Gauge` | 职业量谱（BRDGauge, PLDGauge...） |
| `HasXXX` | Buff 是否激活 |
| `HasXXXOnTarget` | 目标上 DoT 是否由自己施加 |

## ACR 开发包

开发 HiAuRo ACR 推荐用 [HiAuRo.Sdk](https://github.com/denghaoxuan991876906/HiAuRo) NuGet 包——含框架 + 本库 + OmenTools + Dalamud，一键安装。
